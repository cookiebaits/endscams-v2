...
export function exportRecordsToExcel(
  ...
  const exportRows = records.map((r, index) => {
    return {
      ...
      'Post Date': r.postDate || (r.detectedAt ? getPSTDateStamp(new Date(r.detectedAt)) : getPSTDateStamp()),
      'Number Status (Active/Down)': r.isNumberDown ? 'Down / Inactive' : 'Active',
      'Marked Down At': r.numberDownAt || '',
      'Retention Policy': isPrizeOrExtendedRetentionRecord(r) ? '6 Months (Prize / PCH / Stake.us Scam)' : '90 Days Standard',
  ...
  const metaRows = [
    ...
    { 'Property': 'Retention Policy', 'Value': '90-Day Historical Database (Tiered: 6-Mo Prize/PCH/Stake)' },
  ];
