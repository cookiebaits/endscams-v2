      ...
      <div className="w-full px-2 sm:px-4 lg:px-6">
        <div className="flex items-center justify-between h-20 gap-2">
          ...
          <div className="flex items-center h-full shrink-0">
            <Link to="/" className="flex items-center hover:opacity-80 transition-opacity relative z-50 w-[180px] sm:w-[220px] md:w-[240px] lg:w-[280px] xl:w-[340px] shrink-0 h-20">
              <img
                src="/logo.png"
                alt="Cyberscam Watchdog Logo"
                className="absolute -top-[5px] left-0 h-[4.5rem] sm:h-[5.5rem] md:h-[6rem] lg:h-[7rem] xl:h-[7.8rem] max-w-none drop-shadow-md origin-top-left object-contain"
              />
            </Link>
          </div>

          {/* Center section: Nav Links */}
          <div className="hidden lg:flex items-center justify-center gap-1 xl:gap-2 shrink-0 whitespace-nowrap">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`flex items-center gap-1.5 px-2.5 xl:px-3 py-2 rounded-lg font-medium text-xs xl:text-sm transition-all shrink-0 ${
                  isActive(link.to)
          ...
          {/* Right section: Actions (Centered & Responsive) */}
          <div className="flex items-center justify-end gap-1.5 sm:gap-2 shrink-0 whitespace-nowrap">
            <Link
              to="/shutdown"
              className="hidden sm:inline-flex items-center justify-center gap-1.5 px-3 lg:px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-xs lg:text-sm transition-colors shadow-sm shrink-0 whitespace-nowrap"
            >
              <ShieldAlert className="w-3.5 h-3.5 lg:w-4 lg:h-4 shrink-0" />
              <span>Been Scammed? Click here</span>
            </Link>
            <a
              href="https://endscams.org/donate"
              className="hidden md:inline-flex items-center justify-center px-3 lg:px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg font-bold text-xs lg:text-sm transition-colors shadow-sm shrink-0 whitespace-nowrap"
              target="_blank"
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shrink-0"
