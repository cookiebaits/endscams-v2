...
import { RefreshCw, ExternalLink, Phone, Calendar, Tag, AlertTriangle, Loader2, Paperclip } from 'lucide-react';
...
const CATEGORIES = ['All', 'Invoice / Imposter Scam', 'Emergency Scam', 'Lottery / Prize Scam', 'Government Impersonation', 'Spiritual / Spellcaster Scam', 'Crypto Recovery Scam', 'User Report'];
...
export default function TrackerPage() {
    ...
    try {
      const [trackerRes, reportsRes] = await Promise.all([
        supabase
          ...
          .select('id,phone_number,phone_digits,source_name,source_url,report_date,category,description,created_at')
  ...
  const filtered = category === 'All' ? entries : entries.filter(e => e.category === category || (category === 'User Report' && e.type === 'user'));
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 card p-10">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No entries yet</h3>
...
function TrackerEntryCard({ entry }: { entry: CombinedEntry }) {
  const categoryColors: Record<string, string> = {
    ...
    'Spiritual / Spellcaster Scam': 'text-purple-400 bg-purple-500/10',
  ...
  return (
    <div className="card p-5 hover:border-brand-500/30 transition-all duration-200 group">
