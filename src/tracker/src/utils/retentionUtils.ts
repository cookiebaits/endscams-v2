...
// Standard 60-Day Retention constant in milliseconds
 ...
 * - 60 days for all other scam categories
 */
export function getRecordRetentionMs(record: Partial<ScamPhoneRecord> | null | undefined): number {
  return isPrizeOrExtendedRetentionRecord(record) ? SIX_MONTHS_MS : SIXTY_DAYS_MS;
}
...
export function getRecordRetentionLabel(record: Partial<ScamPhoneRecord> | null | undefined): string {
  return isPrizeOrExtendedRetentionRecord(record) ? '6 Months (Prize/Lottery/PCH)' : '60 Days';
}

/**
 * Checks if a record has passed its retention lifespan based on detectedAt
 */
export function isRecordExpired(record: Partial<ScamPhoneRecord> | null | undefined, now: number = Date.now()): boolean {
  if (!record || !record.detectedAt) return false;
  const recordTime = new Date(record.detectedAt).getTime();
  if (isNaN(recordTime)) return false;
  const maxAgeMs = getRecordRetentionMs(record);
