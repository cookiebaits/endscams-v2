...
function mapSupabaseRowToRecord(row: any): ScamPhoneRecord {
  const cleanPhone = row.phone_digits || (row.phone_number ? row.phone_number.replace(/\D/g, '') : '');
  const phoneDisplay = row.phone_number || (cleanPhone ? formatPhoneDisplay(cleanPhone) : 'Unknown');
  ...
  try {
    ...
    sourceDomain = row.source_name || 'Web Search';
  }

  return {
    id: String(row.id || cleanPhone || Math.random().toString(36).substring(2, 9)),
    phone: phoneDisplay,
    cleanPhone,
    scamType: row.category || 'Unknown Scam',
    sourceUrl: row.source_url || 'https://endscams.org',
    sourceDomain: sourceDomain || 'endscams.org',
    platform: row.source_name || 'Harvester Scan',
    snippet: row.description || '',
    detailedSummary: row.description || '',
    searchQuery: 'Harvester Automated Pipeline',
    detectedAt: row.report_date ? new Date(row.report_date).toISOString() : new Date().toISOString(),
    postDate: row.report_date || new Date().toISOString().split('T')[0],
    confidence: 'High',
    isNumberDown: Boolean(row.is_number_down),
    numberDownAt: row.number_down_at,
  };
}
...
export default function App() {
  ...
  const [nextScheduledRefresh] = useState<string>('Today at 1:00 PM PST');
  ...
  const nextScheduledRefreshRef = useRef(nextScheduledRefresh);
  ...
  useEffect(() => {
    syncBridge.init({
      ...
      onPushRecords: (incomingRecords) => {
        setRecords((prev) => {
          ...
          if (newRecords.length > 0) {
            setStatusMessage(`[Sync Bridge] Received & merged ${newRecords.length} synchronized records.`);
      ...
      onTriggerScan: () => {
        setStatusMessage('[Sync Bridge] Remote scan command received.');
        handleRunScanNow();
      },
      ...
      try {
        ...
        if (hour === 7 || hour === 13) {
          ...
          if (!alreadyTriggered && !isScanning) {
            ...
            console.log(`[Auto-Trigger] ${slotLabel} reached! Triggering harvester scan...`);

            setStatusMessage(`[Auto-Scan Active] ${slotLabel} reached — Triggering harvester scan.`);
            handleRunScanNow();
          }
        }
      } catch (err) {
  ...
  // Fetch records directly from Supabase with built-in NoSQL fallback
  const fetchRecords = async () => {
    try {
      ...
      if (!error && Array.isArray(data) && data.length > 0) {
        const mapped = data.map(mapSupabaseRowToRecord);
        setRecords(mapped);
        setLastScanSummary(`Loaded ${mapped.length} active records from Supabase database.`);

        // Mirror into client-side built-in NoSQL database
        noSqlDatabase.getRecordsCollection().clear();
        noSqlDatabase.getRecordsCollection().insertMany(mapped);
        noSqlDatabase.persist();
      } else {
        // Fallback to client NoSQL database store
        const localDocs = noSqlDatabase.getRecordsCollection().getAll();
        if (localDocs.length > 0) {
          setRecords(localDocs);
        }
      }
    } catch (err) {
      console.warn('Supabase fetch failed, utilizing NoSQL database fallback:', err);
      const localDocs = noSqlDatabase.getRecordsCollection().getAll();
      if (localDocs.length > 0) {
        setRecords(localDocs);
      }
  ...
  useEffect(() => {
    fetchRecords();
    const pollInterval = isScanning ? 4000 : 20000;
    const interval = setInterval(fetchRecords, pollInterval);
    return () => clearInterval(interval);
  }, [isScanning]);

  // Trigger manual harvester scan via fetcher service /refresh endpoint
  const handleRunScanNow = async () => {
    setIsScanning(true);
    setScanProgress(10);
    setScanStatusMessage("Initiating harvester scan...");
    setErrorMessage(null);
    setStatusMessage(`Initiating manual harvester scan...`);

    try {
      const fetcherBase = getFetcherUrl();
      const refreshUrl = fetcherBase ? `${fetcherBase}/refresh` : '/refresh';
      ...
      if (response.ok) {
        ...
        setScanProgress(100);
        setStatusMessage(data.message || `Scan completed! ${data.inserted ?? 0} new entries added.`);
        fetchRecords();
      } else if (response.status === 429) {
        setStatusMessage('Harvester scan already in progress or recently executed.');
      } else {
        // Handle standalone or missing backend proxy gracefully
        setStatusMessage('Harvester trigger submitted. Fetching latest database entries...');
        setTimeout(fetchRecords, 2000);
      }
    } catch (err: any) {
      console.warn('Harvester trigger notice:', err);
      setStatusMessage('Trigger complete. Refreshing database records...');
    ...
    if (target) {
      try {
        await supabase
          .from('tracker_entries')
          .update({
            description: `${target.detailedSummary || target.snippet} [NUMBER DOWN MARKED]`
          })
          .eq('phone_digits', target.cleanPhone);
      } catch (err) {
        console.warn('Number down sync notice:', err);
      }
    }
  };
  ...
  const handleAddManualRecord = async (recordData: Omit<ScamPhoneRecord, 'id' | 'detectedAt'>) => {
    const cleanPhone = recordData.cleanPhone || recordData.phone.replace(/\D/g, '');
    const newRecord: ScamPhoneRecord = {
      ...
      sourceDomain: recordData.sourceDomain || 'endscams.org',
      searchQuery: recordData.searchQuery || 'Manual User Report',
    };

    // Update local state and NoSQL database immediately
    setRecords((prev) => [newRecord, ...prev]);
    noSqlDatabase.getRecordsCollection().insertOne(newRecord);
    noSqlDatabase.persist();
    setStatusMessage(`Added manual scam record for ${newRecord.phone}. Data retained.`);

    // Sync to Supabase `tracker_entries` table
    try {
      ...
      await supabase.from('tracker_entries').upsert(
        {
          ...
          source_name: newRecord.platform || 'User Report',
          source_url: newRecord.sourceUrl || 'https://endscams.org',
          report_date: newRecord.postDate,
          category: newRecord.scamType,
          description: newRecord.detailedSummary || newRecord.snippet || 'User reported scam number',
          expires_at: expiresAt.toISOString(),
        },
        { onConflict: 'phone_digits,source_name' }
      );
    } catch (err) {
      console.warn('Supabase sync notice on manual record add:', err);
    }
  };
