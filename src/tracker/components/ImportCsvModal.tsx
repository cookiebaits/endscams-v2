...
export const ImportCsvModal: React.FC<ImportCsvModalProps> = ({
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      // 1. Attempt to sync with dedicated /api/records/import-csv (atomic disk, seed, and Supabase sync)
      try {
        let response = await fetch('/api/records/import-csv', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          try {
            ...
            if (data.supabase?.success) {
              serverMessage = `Synced ${data.total} records to database & Supabase.`;
    ...
    if (existingRecords.length > 0) {
      ...
      const sampleContent =
        ...
        [
          ...
          '"Tech Support / Geek Squad Impersonation","1 (812) 552-9820","18125529820","Geek Squad","2026-09-10","https://techscammersunited.com/t/geek-squad/123","Tech Support United","US","Fake invoice renewal email","Active Line"',
        ].join('\r\n');
