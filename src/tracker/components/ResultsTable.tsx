...
export const ResultsTable: React.FC<ResultsTableProps> = ({
  ...
  const datesList = useMemo(() => {
    ...
    records.forEach((r) => {
      const day = r.postDate || (r.detectedAt ? getPSTDateStamp(new Date(r.detectedAt)) : '');
      if (day) {
        dates.add(day);
      }
    });
    ...
    const sorted = records
      .filter((record) => {
        ...
        if (filters.dateFilter && filters.dateFilter !== 'ALL') {
          ...
          const recDay = record.postDate || (record.detectedAt ? getPSTDateStamp(new Date(record.detectedAt)) : '');
          if (filters.dateFilter === 'TODAY') {
            const todayStr = getPSTDateStamp();
          ...
          } else if (filters.dateFilter === '60D' || filters.dateFilter === '90D') {
            const limitDays = filters.dateFilter === '60D' ? 60 : 90;
            if (now - recTime > limitDays * 24 * 60 * 60 * 1000) return false;
  ...
  // Copy helper
  const handleCopyText = (text: string, id: string, isPhone = false) => {
    const textToCopy = isPhone ? getCleanCopyPhone(text) : text;
      ...
      {isMobileActive ? (
        ...
        <>
                      ...
                      >
                            ...
                            <div className="space-y-1.5 cursor-help">
                              <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                {isPrizeOrExtendedRetentionRecord(record) ? (
                                  ...
                                  <span className="inline-flex items-center px-1.5 py-0.5 bg-slate-800/80 text-slate-400 border border-slate-700/60 rounded text-[9px] font-medium" title="Standard Scam Record (90-Day Auto-Retention)">
                                    90-Day Retained
                                  </span>
                                )}
                              </div>
                        ...
                        <td className="p-3.5 whitespace-nowrap">
                          <div className="flex items-center space-x-2">
                            ...
                            {/* Copy Phone Button */}
                            <button
                              onClick={() => handleCopyText(record.phone, `phone-${record.id}`, true)}
          ...
          <div className="p-3 bg-slate-950 border-t border-slate-800 flex items-center justify-between text-xs text-slate-400">
            <div>
              Showing <strong>{processedRecords.length}</strong> of <strong>{records.length}</strong> entries (Tiered retention: 6-mo for PCH, Mega Millions, Reader's Digest, Stake.us & prize scams; 90-day standard)
              {selectedIds.length > 0 && (
            ...
            <form onSubmit={handleThreatSearchSubmit} className="space-y-4">
              ...
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-1 text-xs text-slate-400">
                ...
                <p className="text-[11px]">
                  Discovered numbers will be incrementally added onto your existing retained database. PCH, Mega Millions, Reader's Digest, Stake.us, and prize scams are preserved for up to 6 months; all other scam categories remain safely retained for up to 90 days.
                </p>
              </div>
