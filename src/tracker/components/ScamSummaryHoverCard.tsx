...
export const ScamSummaryHoverCard: React.FC<ScamSummaryHoverCardProps> = ({
  ...
  const handleCopy = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigator.clipboard.writeText(getCleanCopyPhone(record.phone));
    setCopied(true);
  ...
  const getDerivedCompany = () => {
    return resolveTargetCompany(record.impersonatedCompany, record.scamType, record.snippet);
  };
  ...
  const getDerivedAmount = () => {
    ...
    if (match) return match[1];
    return 'Variable Fee / Advance Fee';
  };
  ...
  const popoverContent = isOpen && typeof document !== 'undefined' ? (
    createPortal(
      ...
      >
        ...
        {/* Retention Policy Status Line */}
        <div className="mt-2 px-2.5 py-1.5 bg-slate-950/90 rounded-lg border border-slate-800 flex items-center justify-between text-[10px]">
          <span className="text-slate-400">Archive Retention:</span>
          {isPrizeOrExtendedRetentionRecord(record) ? (
            <span className="flex items-center gap-1 text-amber-300 font-semibold">
              <Gift className="w-3 h-3 text-amber-400" />
              <span>6 Months (Prize / PCH / Stake.us)</span>
            </span>
          ) : (
            <span className="text-slate-300 font-medium">90 Days (Standard)</span>
          )}
        </div>
