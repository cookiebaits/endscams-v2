...
export { formatDisplayPhone as formatPhoneDisplay } from '../utils/phoneUtils';
  ...
  get(_target, prop) {
    ...
    if (prop === 'from') {
      return (_table: string) => ({
        select: () => ({
          or: () => ({ gt: () => ({ order: () => Promise.resolve({ data: [], error: null }) }), eq: () => ({ maybeSingle: () => Promise.resolve({ data: null, error: null }) }) }),
